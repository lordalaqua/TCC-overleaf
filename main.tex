...
Considere uma imagem esférica $I(x,y)$, e as estimativas de profundidade de duas seções com sobreposição $D_L(x,y)$ e $D_R(x,y)$, e seja $P$ o conjunto dos pixels e $L$ o conjunto das linhas da região de sobreposição entre as duas seções. Queremos calcular pesos $w_{kL}$ e $w_{kR}$ para cada linha $k$, tal que $ k \in L$ para ponderar as linhas de $D_L(x,y)$ e $D_R(x,y)$. 
