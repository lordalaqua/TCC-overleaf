...
\todo{formalizar esta seção melhor}
...
\begin{figure}
    \caption{Etapas do \textit{pipeline}\todo{Fazer figura com exemplos de imagens intermediárias}}
    \begin{center}
...
Sejam $I(x,y)$ uma imagem esférica $S_L$ e $S_R$ duas seções da esfera $I$, $D_L(x,y)$ e $D_R(x,y)$ estimativas de profundidade de $S_L$ e $S_R$, $P$ o conjunto dos pixels e $L$ o conjunto das linhas da região de sobreposição entre $S_L$ e $S_R$. Queremos calcular pesos $w_{kL}$ e $w_{kR}$ para cada linha $k$, tal que $ k \in L$ para ponderar as linhas de $S_L$ e $S_R$. Para isso, queremos escolher pesos que minimizem a diferença entre as estimativas de profundidade em cada pixel onde há sobreposição, ou seja, minimizar o somatório:
$$ W_D =\sum_{(i,j) \in P}(w_{iL}D_L(i,j)-w_{iR}D_R(i,j)) $$
No entanto, isto pode gerar pesos muito diferentes entre linhas adjacentes nas imagens, gerando descontinuidades no mapa de profundidade, logo é interessante adicionar o seguinte termo ao problema de minimizaçao para manter a coerência entre linhas adjacentes:
...
$\alpha$, $\beta$ e $\gamma$ são constantes, $\alpha$ é um fator de escala para linhas com cor similar, e $\beta$ o fato para linhas com cor não similar. $\gamma$ define um limite da diferença entre dois pixels que define sua similaridade na equação. 
Os pesos são calculados minimizando a soma de $W_D$ e $W_C$, pela decomposição em valores singulares (\textit{Singular Value Decomposition}, ou SVD) da equação na forma matricial:

\todo{forma matricial}

Cada seção da esfera tem duas regiões de overlap, que geram pesos diferentes em cada parte da imagem. É feita uma interpolação destes pesos à medida que se encontra a metade da seção esférica. Cada seção fica então com mapas de disparidades com menos diferenças entre si.
